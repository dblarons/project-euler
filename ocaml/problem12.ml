...
let count_divisors t =
  let max = Float.to_int(sqrt(Int.to_float(t)))
  in List.fold_left
    (fun acc i -> if t mod i = 0 then (acc + 2) else acc)
    0
    (List.init max (fun i -> i + 1))

let triangle_number t =
  List.fold_left (+) 0 (List.init t (fun i -> i + 1))
...
let main () =
  ...
  in Printf.printf "n: %d | t: %d | d: %d" number triangle divisors;
